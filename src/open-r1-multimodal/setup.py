...
#   * These pins are intentionally hardcoded to the known-good environment for this repo.
_deps = [
    "accelerate==1.10.1",
    "bitsandbytes==0.47.0",
    "black==25.1.0",
    ...
    "math-verify==0.8.0",  # Used for math verification in grpo
    "packaging==25.0",
    "parameterized==0.9.0",
    "pytest==8.4.1",
    "safetensors==0.6.2",
...
deps = {normalize_package_name(b): a for a, b in (re.findall(r"^(([^!=<>~ \[\]]+)(?:\[[^\]]+\])?(?:[!=<>~ ].*)?$)", x)[0] for x in _deps)}


def deps_list(*pkgs):
    return [deps[normalize_package_name(pkg)] for pkg in pkgs]
