...
class VLMGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        #NOTE 查看待选图像和历史数据图像路径都是什么
        # X_0 = inputs
        # print("Input2==="*20)
        # print("len_X_0:", len(X_0))
        # for x_0 in X_0:
        #     print(x_0["imageB_path"])
        #     print(x_0["imageA_path"])
        #     print(x_0["preferred_images_path"])
        #     print(x_0["non_preferred_images_path"])
        # print("Input2==="*20)
        #NOTE 查看待选图像和历史数据图像路径都是什么
        ...
        #NOTE 查看VLM的prompt都是什么
        # print("prompts_text3==="*20)
        ...
        #=============================Put the images into the VLM, need to change========================
        images = []
        for x in inputs:
            if "preferred_images_path" in x and x["preferred_images_path"] is not None:
                prefer_imgs = [PIL.Image.open(p) for p in self._get_key_from_inputs(x, "preferred_images_path")]
                # print("length of prefer_imgs:", len(prefer_imgs))
            if "non_preferred_images_path" in x and x["non_preferred_images_path"] is not None:
                non_prefer_imgs = [PIL.Image.open(p) for p in self._get_key_from_inputs(x, "non_preferred_images_path")]
                # print("length of non_prefer_imgs:", len(non_prefer_imgs))
            if "imageA_path" in x and x["imageA_path"] is not None:
                A_imgs = [PIL.Image.open(p) for p in self._get_key_from_inputs(x, "imageA_path")]
                # print("length of A_imgs:", len(A_imgs))
            if "imageB_path" in x and x["imageB_path"] is not None:
                B_imgs = [PIL.Image.open(p) for p in self._get_key_from_inputs(x, "imageB_path")]
                # print("length of B_imgs:", len(B_imgs))

            for image_group in [prefer_imgs, non_prefer_imgs, A_imgs, B_imgs]:
        ...
        prompt_inputs = self.vlm_module.prepare_model_inputs(  #准备模型输入
        ...
        # # NOTE 打印中间结果
        # prompt_inputs_print = prompt_inputs
        # print("***"*20)
        # print("prompt_inputs:",prompt_inputs_print)
        # print("***"*20)
        # # 打印中间结果
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            # # NOTE 打印中间结果
            # print("***"*20)
            # print("unwrap_model_for_generation")
            # print("***"*20)
            # # 打印中间结果
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                # #NOTE 查看输入都是什么
                # print("conversational5==="*20)
                # print("is_conversational(inputs[0]):", is_conversational(inputs[0]))
                # print("conversational5==="*20)
                # #NOTE 查看输入都是什么
                ...
                # #NOTE 查看此刻reward_kwargs都是什么
                # print("reward_kwargs5==="*20)
                ...
                #NOTE 新增加一个键值 step
                # 添加step键（假设step的值需要根据业务逻辑确定，这里以0为例）
                reward_kwargs['step'] = [self._step] * len(inputs)  # 为每个example创建一个step值

                # #NOTE 查看此刻reward_kwargs都是什么
                # print("reward_kwargs6==="*20)
                # print("reward_kwargs:", reward_kwargs)
                # print("reward_kwargs6==="*20)
                # #NOTE 查看此刻reward_kwargs都是什么


                # #NOTE 查看此刻completions都是什么
                # print("completions7==="*20)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None,):
        ...
        #=====NOTE DAPO Token-Level Policy Gradient Loss======
        ...
        # print("coef_1_shape:", coef_1.shape)
        # print("advantages_shape:", advantages.shape)
        # print("per_token_loss_shape:", per_token_loss.shape)
        # print("completion_mask_shape:", completion_mask.shape)
        # print("prompt_ids:",prompt_ids.shape)
        # print("completion_ids:",completion_ids.shape)
        # print("prompt_mask:",prompt_mask.shape)
        # print("self_batch_size:", batch_size)
        # print("XSQ==="*20)
        #NOTE ============debug====================
        ...
        # loss = ((per_token_loss * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()  #Sequence-level loss 先计算token 再计算 Sequence

        # Log clip ratio  #NOTE 容易出bug 因为completion_mask发生了变化
